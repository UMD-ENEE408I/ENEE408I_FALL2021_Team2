...
float base_vel = 36;    //use for both left and right 
...
//Gyroscope Variables
float gx, gy, gz;
float gz_offset = 0;    //initialize the gyroscope offset to 0 - to be updated in setup
...
while(turn > 0){
 //Gyroscope angle calculation
  ...
  if (gz_deg_adjusted <= 65)    //Ashley's Robot needs this to be 74
  {
    turnLeft();
  }
  else
...
while(turn > 0){
 //Gyroscope angle calculation
  ...
  if (gz_deg_adjusted >= -140)    //Ashley's Robot needs this to be 74
  {
    turn_180();
  }
  else
...
void Junction(int sensor_count, int left_count, int right_count){
  if(sensor_count > 5){  
    if(right_count > 4 && left_count < 4){
     //Serial.println("right");
      PID_Motor_Controller(20,20);    //this might help with making the turning more precise
      delay(100);
      moveStop();
      Right_Turn();
      //resetting the motor controller variables after the turn is executed
      left_position_setpoint = enc1.read();
      right_position_setpoint = -enc2.read();
      motor_last_time_s = micros()/1e6;
      left_last_error = 0;
      right_last_error = 0;  
    }
    if(right_count < 4 && left_count > 4){
...
void loop() 
{
 ...
 // Serial.print(center_pos); Serial.print("\t");
  ...
  if (main_error_sig == 7) //stop the robot when it's off the line    //add case here for turning around - the 180 degree turn case in the junction section will never be reached
  {
      sharp_turnLeft();
      delay(75);
      moveStop();
  ...
  else
  {
   // Serial.print(micros());
    ...
    Junction(sensor_count, left_sensor_count, right_sensor_count);
  }
 // Serial.println();
}
