...
const float base_vel = 20;    //use for both left and right 
float main_last_error = 0;
...
void PID_Motor_Control(float left_control_sig, float right_control_sig)   //just testing the left in this code for now
{
  analogWrite(M1_IN_1, 0);
  analogWrite(M1_IN_2, left_control_sig);
  analogWrite(M2_IN_1, 0);
  analogWrite(M2_IN_2, right_control_sig);
}
...
void loop() 
{
  ...
  int main_error_sig;    // error signal 
  ...
  {
    P = error_sig;
    main_last_error = main_error_sig;
  ...
  {
    Motor_Controller(left_vel,right_vel);
  }
  Serial.println();
}
